...
impl Emulator {
    ...
    fn V(&mut self, reg: u8) -> &mut u8 {
        &mut self.V[reg as usize]
    }
    ...
    pub fn tick(&mut self) -> Result<(), RuntimeError> {
        ...
        debug!("| 0x{PC:X} | {opcode}", PC = self.PC);
        ...
        match nibbles {
            ...
            (0x2, _, _, _) | (0, _, _, _) => self.call(opcode.address())?,
            // Skips the next instruction if VX equals opcode lower 8-bits
            (0x3, x, _, _) => {
                if *self.V(x) == opcode.kk_byte() {
                    self.PC += 2
                }
            }
            ...
            // Sets VX to NN.
            (0x6, x, _, _) => *self.V(x) = opcode.kk_byte(),
            // Adds NN to VX (carry flag is not changed).
            (0x7, x, _, _) => *self.V(x) = self.V(x).wrapping_add(opcode.kk_byte()),
            // Sets VX to the value of VY.
            (0x8, x, y, 0x0) => *self.V(x) = *self.V(y),
            // Sets VX to VX or VY. (bitwise OR operation)
            (0x8, x, y, 0x1) => *self.V(x) |= *self.V(y),
            // Sets VX to VX and VY. (bitwise AND operation)
            (0x8, x, y, 0x2) => *self.V(x) &= *self.V(y),
            // Sets VX to VX xor VY.
            (0x8, x, y, 0x3) => *self.V(x) ^= *self.V(y),
            // The values of Vx and Vy are added together. If the result is greater than 8 bits (i.e., > 255,)
            // VF is set to 1, otherwise 0. Only the lowest 8 bits of the result are kept, and stored in Vx.
            (0x8, x, y, 0x4) => {
                let result = *self.V(x) as u16 + *self.V(y) as u16;
                *self.V(x) = (result & 0xFF) as u8;
                self.V[FLAGS_REGISTER] = if result & 0xFF00 != 0 { 1 } else { 0 }
            }
            // Set Vx = Vx - Vy, set VF = NOT borrow.
            // If Vx > Vy, then VF is set to 1, otherwise 0. Then Vy is subtracted from Vx, and the results stored in Vx.
            (0x8, x, y, 0x5) => {
                self.V[FLAGS_REGISTER] = if *self.V(x) > *self.V(y) { 1 } else { 0 };
                *self.V(x) = self.V(x).wrapping_sub(*self.V(y));
            }
            // Set Vx = Vx SHR 1.
            // If the least-significant bit of Vx is 1, then VF is set to 1, otherwise 0. Then Vx is divided by 2.
            (0x8, x, _, 0x6) => {
                self.V[FLAGS_REGISTER] = *self.V(x) & 1;
                *self.V(x) = *self.V(x) >> 1;
            }
            ...
            (0x8, x, _, 0xE) => {
                self.V[FLAGS_REGISTER] = (*self.V(x) >> 7) & 1;
                *self.V(x) = *self.V(x) << 1;
            }
            ...
            // Set I = nnn.
            // The value of register I is set to opcode address.
            ...
            (0xC, x, _, _) => *self.V(x) = self.rand.next() & opcode.kk_byte(),
            ...
            (0xF, x, 0x0, 0x7) => *self.V(x) = self.delay_timer,
            // Wait for a key press, store the value of the key in Vx.
            // All execution stops until a key is pressed, then the value of that key is stored in Vx.
            (0xF, x, 0x0, 0xA) => self.state = State::WaitingKey { x },
            // Set delay timer = Vx.
            // DT is set equal to the value of Vx.
            (0xF, x, 0x1, 0x5) => self.delay_timer = *self.V(x),
            // Set sound timer = Vx.
            // ST is set equal to the value of Vx.
            (0xF, x, 0x1, 0x8) => self.sound_timer = *self.V(x),
            // Set I = I + Vx.
            // The values of I and Vx are added, and the results are stored in I.
            (0xF, x, 0x1, 0xE) => self.I += *self.V(x) as Address,
            // Set I = location of sprite for digit Vx.
            (0xF, x, 0x2, 0x9) => self.I = *self.V(x) as Address * 5,
    ...
    #[inline]
    fn call(&mut self, address: Address) -> Result<(), RuntimeError> {
        self.stack.push(self.PC)?;
        Ok(self.PC = address) // Assing returns ()
    }
